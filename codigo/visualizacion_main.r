...
ui <- fluidPage(
    ...
    tabsetPanel(  id = "mainTabs",
        tabPanel("Introducción"
                ...
                # Logo ARH fijo abajo a la izquierda
                tags$div(
                    style = "position: fixed; bottom: 8px; right: 8px; z-index: 1000;",
                    tags$img(src = path_logoARH, alt = "Logo ARH", style = "height:60px; max-width:120px;")
                )
        ),
        ...
        tabPanel("Selección femenina",
            ...
            column(8,
                # Panel central: gráfico de evolución temporal según filtros (centrado)
                div(style = "display:flex; justify-content:center; align-items:center; height:100%;",
                    div(style = "width:100%; max-width:900px;",
                        plotOutput("fem_evolucion", height = "500px")
                    )
                )
            )
            )
        ...
        tabPanel("Selección masculina",
            ...
            fluidRow(
                ...
                column(8,
                    div(style = "display:flex; justify-content:center; align-items:center; height:100%;",
                        div(style = "width:100%; max-width:900px;",
                            plotOutput("masc_evolucion", height = "500px")
                        )
                    )
                )
            )
...
server <- function(input, output) {
    ...
    datos_filtrados_fem <- reactive({
        req(input$fem_tipo_torneo, input$fem_tipo_rival)
        datos_fem %>%
    ...
    ## Plot evolucion
    output$fem_evolucion <- renderPlot({
       crear_grafico_evolucion_por_anio(datos_filtrados_fem(), input$fem_que_visualizar)
    })

    # Panel masc
    ## Filtrar datos según inputs
    datos_filtrados_masc <- reactive({
        req(input$masc_tipo_torneo, input$masc_tipo_rival)
        datos_masc %>%
    ...
    ## Plot evolucion
    output$masc_evolucion <- renderPlot({
        crear_grafico_evolucion_por_anio(datos_filtrados_masc(), input$masc_que_visualizar)
    })
}
